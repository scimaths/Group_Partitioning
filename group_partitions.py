...
def generate_2_allocations(num: int):
    ...
    for i in range(2**(num-1)):
            ...
            yield allocation.astype(np.int32)
...
def utilities_adj(graph_adj: np.ndarray, allocation: np.ndarray):
    ...
    envy = np.maximum(0, cross_utilities - np.diag(cross_utilities).reshape(-1, 1)) * (1 - co_allocation)
    return {
...
if __name__ == "__main__":
    ...
    test_ef_max(args.nodes, args.num_graphs, args.seed)